/**
 * confirm-pipeline — pure (non-HTTP) confirmation flow.
 *
 * Extracted from `src/app/api/negotiate/confirm/route.ts` per proposal
 * `2026-04-19_confirm-pipeline-extraction_reviewed-2026-04-20_decided-2026-04-20.md`.
 * The HTTP route is now a thin wrapper: body-parse → `confirmBooking` →
 * map `ConfirmResult` to `NextResponse` → write `ConfirmAttempt` in
 * `finally`. MCP's `propose_lock` tool endpoint calls this module directly.
 *
 * Reliability invariants carried over unchanged:
 *   1. Session status transition `active → agreed` uses a compare-and-swap
 *      (`updateMany where status != 'agreed'`) so concurrent calls can't
 *      both succeed. Loser returns winner's data (same slot) or a
 *      `slot_mismatch` refusal (different slot).
 *   2. `waitUntil(extractLearnings(...))` — deferred Claude call, not in
 *      critical path (design note C1 in the proposal).
 *   3. Independent post-GCal work runs in parallel.
 *   4. The `isSynthetic` guard at the GCal branch logs to `RouteError` for
 *      `/admin/failures` triage; passes `userAgent` through when available.
 *
 * Silent partial-failure classes that carry over UNCHANGED (per proposal
 * C2 — "behavior-preserving"):
 *   - `NegotiationOutcome.create` swallow-on-throw (was route.ts:490-512).
 *   - Guest-flow onboarding nudge try/catch (was route.ts:574-619).
 *   - Stretch-booking channel notification try/catch (was 515-538).
 *   - Schedule-cache invalidate non-blocking failure (was 428-430).
 * None are fixed here; noted so follow-up reviewers don't relitigate.
 */
...
export type ConfirmInput = {
  sessionId: string;
  /** ISO dateTime; may or may not carry offset (DST-corrected inside). */
  dateTime: string;
  /** Minutes; default 30. */
  duration?: number;
  /** "video" | "phone" | "in-person"; default "video". */
  format?: string;
  location?: string | null;
  guestEmail?: string | null;
  guestName?: string | null;
  /** Preserve `undefined`-is-no-op semantics (N4) — only written when a
   *  boolean is explicitly passed. */
  wantsReminder?: boolean;
  guestNote?: string | null;
  /** HTTP-caller metadata — plumbed through for `RouteError` context on the
   *  synthetic-in-prod branch. MCP callers pass null. */
  userAgent?: string | null;
};

/** Pinned to the Prisma enum so drift is impossible (C3). */
type AttemptOutcome = Prisma.ConfirmAttemptCreateInput["outcome"];

export type ConfirmAttemptRecord = {
  outcome: AttemptOutcome;
  /** Free-text error detail for `ConfirmAttempt.errorMessage` — null on success. */
  error: string | null;
 ...
 *
 * `ok: true` means the session is in `agreed` state after this call. The
 * optional `warnings[]` surfaces non-fatal degraded paths (`gcal_failed`
 * = DB transition landed but GCal event didn't; admin-failures surfaces
 * it via `RouteError` + `ConfirmAttempt.outcome = "gcal_failed"`).
 *
 * `ok: false` means no booking state change was persisted (or, for
 * `slot_mismatch`, a different caller won the CAS and this call's slot
 * was rejected). Route maps `reason` to HTTP status.
 */
export type ConfirmResult =
  | {
      ok: true;
      outcome: "success" | "already_agreed";
      status: "confirmed";
      dateTime: string;
      duration: number;
      format: string;
...
// ---------------------------------------------------------------------------
// DST-correct offset helper (moved verbatim from route.ts)
// ---------------------------------------------------------------------------
 ...
 */
export function computeUtcOffset(tz: string, date: Date = new Date()): string {
  const now = date;
...
export async function confirmBooking(input: ConfirmInput): Promise<ConfirmResult> {
  ...
  // Attempt-tracking state — surfaced via ConfirmResult.attempt.
  let attemptOutcome: AttemptOutcome = "server_error";
  let attemptError: string | null = null;
  let attemptSessionId: string | null = null;
  let attemptSlotStart: Date | null = null;
  let attemptSlotEnd: Date | null = null;

  const buildAttempt = (): ConfirmAttemptRecord => ({
    outcome: attemptOutcome,
    error: attemptError,
  ...
  // NOTE: `timezone` from the input is never read (C4 / ground-truth
  // protocol). The host's timezone is canonical and comes from stored
  // preferences. LLMs must not be trusted to emit IANA strings.
  const {
    sessionId,
    dateTime,
    duration,
  ...
  //
  // When link.parameters.partialAttendance === "allowed", the host has opted in to
  ...
  const linkLocation =
    typeof linkRulesObj.location === "string" && linkRulesObj.location.trim()
      ...
      : null;
  // negotiatedLocation (guest-locked) takes priority over link.parameters.location.
