...
| **Group coordination MCP tools** (`create_group_link`, `get_group_responses`). Group links (`NegotiationLink.mode: "group"`) are live in the product but not yet exposed on the `/api/mcp/host` surface — host agents can't create group links or read participant responses. See [`GROUPLINKS.md`](../../../../GROUPLINKS.md) §8 for the full owed list. | No blocker; sequenced after PR-2/PR-3 host tools | [`GROUPLINKS.md`](../../../../GROUPLINKS.md) §8 |
