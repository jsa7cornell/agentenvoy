...
- update_time: Propose new time or change duration → {"action":"update_time","params":{"sessionId":"...","dateTime":"...","timezone":"...","duration":50}}
  ...
  - **Only use `dateTime` when re-proposing to a guest who has already engaged.** If the link has no guest activity yet (draft/pre-engagement), the server will reject a dateTime-setting call. For draft-stage intents like "push it to Wednesday" or "make it next week", adjust the link's offer via `update_link` fields (dateRange, preferredDays, preferredTimeStart/End) — never synthesize a specific dateTime from a window-shaped request.
