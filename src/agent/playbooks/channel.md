...
- create_link: Create a new invite → {"action":"create_link","params":{"inviteeName":"...","topic":"...","format":"...","duration":45,"minDuration":30,"isVip":true,"urgency":"asap","intent":{"steering":"open"},"rules":{"preferredDays":["Mon"],"dateRange":{"start":"YYYY-MM-DD","end":"YYYY-MM-DD"},"location":"Coupa Cafe, Palo Alto"}}}
  ...
  - **Time-of-day windows — single vs multi.** Both fields live under `rules`, both are 24h `HH:MM` strings, both apply across whatever `dateRange` / `preferredDays` you also set.
...
- expand_link / update_link: Adjust an EXISTING link's offer (same handler, either name works) → {"action":"update_link","params":{"code":"hhkkkw","preferredTimeStart":"06:00"}} or {"action":"update_link","params":{"code":"hhkkkw","allowWeekends":true}} or {"action":"update_link","params":{"code":"hhkkkw","dateRange":{"start":"2026-04-22","end":"2026-04-24"},"duration":45}}. Use this for draft-stage edits ("push it to Wednesday", "open up to 6am", "include weekends", "make it 45 min on the link") and whenever the host is tweaking the link's offer rather than re-proposing a specific slot. Never infer hours the host didn't name.
