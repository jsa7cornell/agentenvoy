...
- `update_link` — `{"action":"update_link","params":{"code":"<linkCode>","activity":"drinks","availability":[{"days":[1,2,3,4,5],"startMinutes":540,"endMinutes":1080},{"days":[1,2,3,4,5],"startMinutes":1020,"endMinutes":1320}],"blockedRanges":[{"start":"...","end":"..."}]}}` — edit the link's offer (activity, format, duration, location, time-of-day windows, day exclusions, one-off date blocks, invitees). `availability` is `AvailabilityWindow[]` (`{days: number[] 0=Sun..6=Sat, startMinutes, endMinutes}`); pass the COMPLETE replacement array. Use when the host's edit is to the link itself, not just the current session. The full field surface and authoring guidance — including the AVAILABILITY FORMAT section and the time-of-day vocabulary table — live in `calendar-event-composer.md` (the host's dashboard chat composer); the same rules apply here.
