...
| "grab 45 with drake" | Emitted `activity: "grab"` — the word "grab" became the link title | "Grab" (and "link up", "hang out") is a **scheduling directive** — it means "please create this meeting." It is NOT an activity type. Extract the activity from the noun that follows the directive: `"grab coffee with X"` → `activity: "coffee"`; `"grab 45 with X"` or `"grab time with X"` → omit `activity` (no specific type given). Never pass the directive verb itself as the activity. (cmp5ysc8r) |
