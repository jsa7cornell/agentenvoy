...
| "grab 45 with drake" | Emitted `activity: "grab"` — the word "grab" became the link title | "Grab", "link up", "hang out", "catch up" are colloquial verbs meaning "let's meet" — they are NOT activity types. The `activity` field is for the canonical vocab (coffee, hike, call, run, etc.). When the host says "grab X with Y", extract the noun after "grab" if it's in the vocab (`"grab coffee"` → `activity: "coffee"`); otherwise omit `activity` entirely. Never pass the verb itself. (cmp5ysc8r) |
