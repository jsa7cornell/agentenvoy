/**
 * Dev-only audit dashboard for the side-effect dispatcher.
 *
 * Shows the last 100 SideEffectLog entries. Useful on preview deploys to
 * confirm "what would have been sent" without actually sending, and in
 * local dev to inspect payload shapes.
 *
 * Route is 404ed in production — side-effect payloads can contain real
 * user data and should not leak to live users.
 */
...
export default async function SideEffectsPage({
  ...
  if (process.env.NODE_ENV === "production") {
    // Don't leak internals to real users. Preview deploys have
    // NODE_ENV=production too, but Vercel sets VERCEL_ENV distinctly —
    // allow preview so the whole point of this dashboard (seeing what
    // would have been sent on preview) works.
...
function Stat({
  ...
  label: string;
  value: number;
  tone?: "default" | "green" | "amber" | "blue" | "red" | "gray";
}) {
  const toneClass = {
...
function FilterLink({
  ...
  const active =
    (!param && Object.keys(current).length === 0) ||
    (param && value && current[param] === value);
  return (
    <Link
      href={`/dev/side-effects${qs ? `?${qs}` : ""}`}
